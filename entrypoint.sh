...
/bumper guard "${RELEASE_BRANCH}" "${GITHUB_EVENT_PATH}" "${GITHUB_URL}"
if [ $? -eq 78 ]
then
...
then
    ...
    LATEST_TAG=$(/bumper latest-tag "${GITHUB_REPOSITORY}" "${GITHUB_TOKEN}" "${GITHUB_URL}")

    echo ::debug ::Executing bumper increment github_event_path=${GITHUB_EVENT_PATH}
    INCREMENT=$(/bumper increment "${GITHUB_EVENT_PATH}" "${GITHUB_URL}")
